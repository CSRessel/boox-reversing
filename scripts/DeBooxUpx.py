...
# Vendored from https://github.com/Hagb/decryptBooxUpdateUpx#
# ef0649476f3a13474744393116268241bfb2aa9e
...
try:
    ...
    if version_info[0] == 2:
        raise SystemExit(
            "Need either `pycryptodome` or `pycryptodomex`," " NOT `pycrypto`!"
        )
import csv
        ...
        while block:
            ...
            if not header_checked:
                if decrypted_block[:4] != b"\x50\x4b\x03\x04":
                    raise ValueError(
                        "The decrypted data seems not a zip package, "
